...
private:
    ...
    /// @brief A simple vector containinf the radius of each level in the octree to speed up computations.
    std::vector<double> precomputedRadius;
    ...
    size_t computeMemoryFootprint() {
        ...
        memory += vectorMemorySize(precomputedRadius);
        return memory;
    }
...
public:    
    ...
    explicit LinearOctree(std::vector<Point_t>& points,
        ...
        if (log) {
            ...
            log->minRadiusAtMaxDepth =  precomputedRadius[maxDepthSeen];
        }
    }
    ...
    void setupBbox(InternalPart &inter) {
        precomputedRadius = std::vector<double>(enc.maxDepth() + 1);
        ...
        double maxHalfLength = std::max(halfLengths[0], std::max(halfLengths[1], halfLengths[2]));
        for(int i = 0; i <= enc.maxDepth(); i++) {
            coords_t sideLength = (1u << (enc.maxDepth() - i));
            precomputedRadius[i] = sideLength * maxHalfLength;
        }
    }
    ...
    void computeGeometry(InternalPart &inter) {
        #pragma omp parallel for schedule(static)
            for(uint32_t i = 0; i<inter.prefixes.size(); i++) {
                ...
                centers[i] = enc.getCenter(startKey, level, box, halfLengths, precomputedRadius);
            }
    }
    ...
    [[nodiscard]] std::vector<Point_t*> neighbors(const Kernel& k) const {
        ...
        auto checkBoxIntersect = [&](uint32_t nodeIndex, uint32_t currDepth) {
            ...
            auto nodeRadius = this->precomputedRadius[currDepth];
            switch (k.boxIntersect(nodeCenter, nodeRadius)) {
    ...
    uint32_t getPrecisionLevel(const Vector &toleranceVector) const {
        ...
        for(uint32_t i = 0; i<maxDepthSeen; i++) {
            double radius = precomputedRadius[i];
            double minToleranceVecRadius = std::min(toleranceVector.getX(), std::min(toleranceVector.getY(), toleranceVector.getZ()));
            if(radius < minToleranceVecRadius) {
                precisionLevel = i;
                break;
            }
        }
    ...
    [[nodiscard]] std::vector<Point_t*> neighborsOld(const Kernel& k, Function&& condition) const {
        ...
        auto intersectsKernel = [&](uint32_t nodeIndex, uint32_t nodeDepth) {
            return k.boxOverlap(this->centers[nodeIndex], this->precomputedRadius[nodeDepth]);
        };
        ...
        auto findAndInsertPoints = [&](uint32_t nodeIndex) {
            ...
            auto* endPtr = points.data() + endIndex;
            for (; startPtr != endPtr; ++startPtr) {
                if (k.isInside(*startPtr) && condition(*startPtr)) {
                    ptsInside.push_back(startPtr);
                }
            }
        };
    ...
    void logOctreeBounds(std::ofstream &outputFile, int max_level) {
        ...
        auto logBounds = [&](uint32_t nodeIndex, uint32_t currDepth) {
            auto nodeCenter = this->centers[nodeIndex];
            auto nodeRadii = this->precomputedRadius[currDepth];
